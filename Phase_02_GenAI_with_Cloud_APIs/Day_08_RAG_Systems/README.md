...
- [x] Load `college_notes.csv` into a pandas DataFrame
- [x] Prepare `documents`, `ids`, and `metadatas` lists
- [x] Load the `all-MiniLM-L6-v2` embedding model
- [x] Create ChromaDB client and collection
- [x] Generate embeddings for all 15 notes using `.encode(...).tolist()`
- [x] Add all documents to ChromaDB using `collection.add()`
- [x] Verify with `collection.count()` — should return 15
- [x] Write a `retrieve(question)` function using `collection.query()`
- [x] Write a `build_context(results)` function
- [x] Write a `generate_answer(question, context)` function with grounding system prompt
- [x] Combine into one `ask(question)` function
- [x] Test with an in-scope question — verify correct answer with citation
- [x] Test with an out-of-scope question — verify "I don't know" response
- [x] Test with a cross-subject question — verify correct subject retrieved
...
- [x] I can explain hallucination in my own words
- [x] I can draw the 7-step RAG pipeline from memory
- [x] I ran all notebook cells without errors
- [x] I successfully indexed 15 notes into ChromaDB
- [x] I tested retrieval and saw relevant topics returned at rank 1
- [x] I tested the full pipeline: question → retrieve → inject → answer
- [x] I verified out-of-scope questions return "I don't know"
- [x] I completed the College Knowledge Assistant mini project
- [x] I know at least 2 real-world Data Engineering RAG applications
- [x] I can explain why temperature=0.1 is used for RAG
